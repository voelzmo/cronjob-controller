...
import (
	...
	batch "github.com/voelzmo/cronjob-tutorial-controller/api/v1"
)
	...
	getNextSchedule := func(cronJob *batch.CronJob, now time.Time) (lastMissed time.Time, next time.Time, err error) {
		...
		for t := sched.Next(earliestTime); !t.After(now); t = sched.Next(t) {
			...
			if starts > 100 {
				...
				return time.Time{}, time.Time{}, fmt.Errorf("too many missed start times (>100). Set or decrease .spec.startingDeadlineSeconds or check clock swek")
			}
